...
def sam_seedpoint_prompt(frame, asset):
    seedpoint = asset
    print("Using finger tracking seed point!")

    from segment_anything import sam_model_registry, SamPredictor

    # in contrast to SAM-Track, AR-SAM uses the SAM ViT-H model instead of the SAM ViT-B model
...
def main(take, asset):
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened():
            ...
            if frame_idx == 0:
                pred_mask = sam_seedpoint_prompt(frame, asset)
                torch.cuda.empty_cache()
                gc.collect()
