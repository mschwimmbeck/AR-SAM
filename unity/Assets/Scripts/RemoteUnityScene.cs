...
public class RemoteUnityScene : MonoBehaviour
{
    ...
    uint ProcessMessage(uint command, byte[] data)
    {
        ...
        case  21: ret = MSG_StreamFingerPosition(data); break;
        case  22: ret = MSG_DeleteFingerMark(data); break;
    ...
    // OK
    uint MSG_DeleteFingerMark(byte[] data)
    {
        GameObject go;
        if (GameObject.Find("fingerSphere1"))
        {
            go = GameObject.Find("fingerSphere1");
            Destroy(go);
        }
        GlobalVariables.mark_cnt = 0;
    ...
    uint MSG_StreamFingerPosition(byte[] data)
    {
        int coordinate = BitConverter.ToInt32(data, 0);
        int position = BitConverter.ToInt32(data, 4);

        GameObject go = GameObject.Find("fingerSphere1");

        float pos = go.transform.position[coordinate];
        string pos_string = pos.ToString("F6");

        string[] parts;
        if (pos_string.Contains(","))
        {
        ...
        int intBeforeDecimal = int.Parse(integerPart);
        ...
        if (zeroCount < 6)
        {
            remainingDigits = fractionalPart.Substring(zeroCount);
        }
        else
        {
            zeroCount = 5;
        ...
        {
            res_number = (uint)Math.Abs(intBeforeDecimal);
        }
        else if (position.Equals(1))
        {
            res_number = (uint)zeroCount;
        }
        else if (position.Equals(2))
        {
            res_number = uint.Parse(remainingDigits);
        }
