...
def main(host, take, marker_length, offset_array):
    # General Settings --------------------------------------------------------------------
    # add saving path
    ...
    # set the aruco reference to the tool's tip as based on the transformation determined from the CAD model
    aruco_reference = np.array([[-aruco_half, aruco_half, 0], [aruco_half, aruco_half, 0], [aruco_half, -aruco_half, 0],
                                [-aruco_half, -aruco_half, 0], np.array(offset_array)],
                               dtype=np.float32)
    ...
    def data_recording():
        ...
        while enable:
            ...
            if not corners:
                print("No markers detected. Restarting detection.")
                continue

            if len(ids) > 1:
                print("More than one marker detected. Restarting detection.")
                continue
